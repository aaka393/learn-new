import {
  ...
  export const fetchGalleryItems = async (page = 1, perPage = 12): Promise<{ items: WordPressGalleryItem[], pagination: PaginationInfo } | null> => {
  const response = await fetch(`${WORDPRESS_API_URL}/gallery?_embed&page=${page}&per_page=${perPage}`);
