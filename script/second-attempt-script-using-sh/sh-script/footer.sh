#!/bin/bash
#
# Script to create the Footer content type in Strapi
#
...
if [ -z "$1" ]; then
  echo "Usage: ./footer.sh <JWT_TOKEN> [<filename>]"
  exit 1
fi
...
post_with_retry "$STRAPI_URL/content-type-builder/content-types" '{
  "contentType": {
    ...
    "description": "Footer section with text, description, social links and footer columns",
    "attributes": {
...
FOOTER_RAW_JSON=$(<"$CONTENT_DIR/footer.json")

# Post footer
post_with_retry "$STRAPI_URL/content-manager/collection-types/api::footer.footer" "$FOOTER_RAW_JSON" "sample data in 'Footer'"
