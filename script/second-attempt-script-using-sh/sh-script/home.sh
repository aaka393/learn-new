...
if [ -z "$1" ]; then
  echo "Usage: ./home.sh <JWT_TOKEN> [<filename>]"
  exit 1
fi
...
# if [ -z "$FILENAME" ]; then
#   echo "No filename provided. Processing default content file: home.json"
#   CONTENT_FILE="home.json"
# else
#   echo "Using Filename: $FILENAME"
#   CONTENT_FILE="$FILENAME.json"
# fi
echo "=================================================="
#
# STRAPI_URL="http://localhost:1337"
...
if [[ -z "$IMAGE_HERO" || -z "$IMAGE_SHOWCASE_1" || -z "$IMAGE_SHOWCASE_2" || -z "$IMAGE_STORY" ]]; then
...
HOME_JSON=$(echo "$HOME_RAW_JSON" | jq \
  ...
  '
  .hero.backgroundImage = ($hero | tonumber)
  | .showcases[0].imageUrl = ($sc1 | tonumber)
  | .showcases[1].imageUrl = $sc2
  | .story.backgroundImage = ($story | tonumber)
  ')
