...
const About: React.FC = () => {
  ...
  const team = aboutData.blocks.find((b: any) => b.__component === 'about.team');
  const values = aboutData.blocks.find((b: any) => b.__component === 'about.values');


  console.log(`${baseUrl}${section.image.url}`)
  ...
  return (
    <div className="pt-16 pb-24">
      ...
      {team && (
        <TeamSection
          team={team.teamMembers.map((member: any) => ({
            name: member.name,
            role: member.role,
            bio: member.bio,
            photo: member.photo?.data?.attributes?.url
              ? baseUrl + member.photo.data.attributes.url
              : undefined,
          }))}
        />
      )}
